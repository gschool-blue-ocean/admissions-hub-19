...
async function findOne(req, res, next) {
  if (Number.isNaN(parseInt(req.params.id))) {
    ...
    const result = await db
      .query("SELECT * FROM cohorts WHERE cohort_id=$1", [req.params.id])
      .catch(next);
...
async function remove(req, res, next) {
  if (Number.isNaN(parseInt(req.params.id))) {
    ...
    if (result.rows.length != 1) {
      ...
      await db
        .query("DELETE FROM cohorts WHERE cohort_id = $1", [req.params.id])
        .catch(next);
      res.sendStatus(204);
    }
  }
...
async function update(req, res, next) {
  ...
  if (!haveKeys) {
    ...
    if (result.rows.length != 1) {
      ...
      for (let key in req.body) {
        let queryText = `UPDATE cohorts SET ${key}=$1 WHERE cohort_id = $2`;
